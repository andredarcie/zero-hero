...
export class ItemManager {
  ...
  public loadAuthored(
    ...
    for (const p of list) {
      ...
      this.items.push(new ItemPickup(this.scene, p.type, p.worldX, p.worldY, false, undefined, units));
    }
  }

  /**
   * Drop an item on the ground (a swap): it lands unarmed so it isn't re-collected instantly.
   * `fire` keeps a lit graveto BURNING where it lands — the flame rides the pickup and the
   * fuel keeps counting down.
   */
  public drop(kind: HeldItemKind, worldX: number, worldY: number, fire?: ItemFire, units?: number): void {
    ...
    // `units` explícito vem do pousar do herói (o pacote que estava viajando);
    // sem ele, sementes recém-produzidas (a foice) nascem como um pacote cheio.
    const n = units ?? spawnPackSize(kind);
    this.items.push(new ItemPickup(this.scene, kind, worldX, worldY, true, fire, n));
  }
  ...
  /** A LIT item burns on this tile — a ground torch can spread fire. */
   ...
   *
   * A flor da lua pergunta "ha fogo por PERTO?", que por tile custaria um varrimento de 21 tiles
   * por flor por frame. Um graveto aceso no chao e a mesma tocha que estava na mao e some sozinho
   * quando o combustivel acaba.
   */
  ...
  /**
   * Lift an item off the ground for a manual station or the hero. Returns null when the tile is
   * empty. It respects `isCollectable`, so nothing is taken during its arrival animation.
   */
  public takeAt(x: number, y: number): { kind: HeldItemKind; fire?: ItemFire; units: number } | null {

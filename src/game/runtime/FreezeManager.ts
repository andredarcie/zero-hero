  ...
  public freeze(target: FreezeTarget, ms = FREEZE_MS): boolean {
    ...
    const block = world3d()
      .addBillboard(FX_ICE_TEXTURE, 0, {
        ...
        depthWrite: false,
        alphaTest: 0.02,
        // O gelo VESTE o corpo, e isso pede as DUAS metades da ordem (ver Billboard3D): a camada
        // à frente do plano do ator (DEPTH_LAYER.overlay) ganha o teste de profundidade, e o
        // renderOrder garante que o bloco — que não escreve profundidade — DESENHA depois do
        // corpo. Só com a primeira, o corpo que sai depois pinta por cima mesmo estando atrás.
        depthLayer: 'overlay',
        renderOrder: 3,
      })
      .setTint(ICE_TINT)
